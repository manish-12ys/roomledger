...
class ExpensesScreen extends ConsumerWidget {
  ...
  Future<void> _openAddExpenseSheet(BuildContext context, WidgetRef ref) async {
    // Logic to open sheet (assumed existing in the project)
  }
}
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: _ExpensesSummaryHero(totalPending: totalPending, count: items.length),
          ),
        ),
...
class _ExpensesSummaryHero extends StatelessWidget {
  const _ExpensesSummaryHero({required this.totalPending, required this.count});
      ...
      child: Row(
        children: [
          ...
          const ProgressRing(
            ...
            size: 80,
            strokeWidth: 6,
            child: Icon(Icons.groups_outlined, color: AppTheme.secondary, size: 28),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(value, style: TextStyle(color: color ?? Colors.white, fontSize: 14, fontWeight: FontWeight.w800)),
      ],
    );
  }
}
